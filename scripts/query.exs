...
Enum.each(parsed_response["results"]["bindings"], fn battle ->
  ...
  {:ok, date_time, _} = DateTime.from_iso8601(battle["startTime"]["value"])
  start_date = DateTime.to_date(date_time)

  {:ok, date_time, _} = DateTime.from_iso8601(battle["endTime"]["value"])
  end_date = DateTime.to_date(date_time)
